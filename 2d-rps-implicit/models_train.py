...
from psro_variants.grad_trainer_distributed import psro_subloop_cuda
...
def run_dist_cuda(wandb_run):
    ...
    inner_config = dict(
        ...
        gos_dim = wandb_run.config.gos_dim,
    ...
    for proc in range(wandb_run.config.batch_size):
        ...
        p = tmp.Process(target=psro_subloop_cuda, args=(wandb_run.config.seed, proc, inner_config, game_args_dict, model, game_sampler, meta_grad, stop_value, start_count, end_count, batch_losses, true_losses, batch_norms))
        processes.append(p)
        p.daemon = True
        p.start()
    ...
    for overall_iter in range(wandb_run.config.nb_total_iters):
        ...
        nn.utils.clip_grad_norm_(model.parameters(), wandb.config.outer_gradient_norm)
...
if __name__ == "__main__":
    ...
    hyperparameter_defaults = dict(
        ...
        nb_total_iters = 600,
        nb_psro_iters = 10,
        nb_inner_train_iters = 100,
        nb_exp_iters = 200,
        grad_clip_val = 5,
        clip_correction_val = 0.1,
        agent_nn_input_size = 4,
        model_size = 'small',
        gradient_type = 'implicit',
        distribution_type = 'distributed',
        nb_test_iters = 200,
        test_lr = 0.75,
        norm_break_val = 0.001,
        implicit = True,
        outer_gradient_norm = 0.002,
        seed = 0
    )

    wandb_run = wandb.init(config=hyperparameter_defaults, project='NAC-2d-implicit')
    print(wandb_run.dir)

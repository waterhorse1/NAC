...
def psro_subloop_cuda(seed, proc, config, game_args_dict, model, game_sampler, meta_grad, stop_value, start_count, end_count, batch_losses, true_losses, batch_norms):
    ...
    while stop_value == 0:
        ...
        for psro_iter in range(config['nb_psro_iters']):
            ...
            torch_pop.psro_popn_update(meta_nash, config['nb_inner_train_iters'], config['inner_lr'], config['norm_break_val'], implicit=config['implicit'])
            ...
            if (psro_iter + 1) % config['window_size'] == 0:
                ...
                #print(meta_nash)
                final_exp = torch_pop.get_exploitability(meta_nash, config['nb_exp_iters'], config['norm_break_val'], exp_inner_lr, test=False, implicit=config['implicit'])

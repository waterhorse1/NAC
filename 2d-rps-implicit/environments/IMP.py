...
class TorchPop_imp(nn.Module):
    ...
    def step(self, theta1, theta2, grad=False, meta_nash1=None, meta_nash2=None):
        if not grad:
            # just to evaluate progress without gradient:
            (s1, s2), _ = self.imp.reset()
            dice_memory = Memory()
            for t in range(self.len_rollout):
                if isinstance(theta1, list):
                    a1, lp1 = self.act_aggregation(s1, theta1, meta_nash1)
                else:
                    a1, lp1 = self.act(s1, theta1)

                if isinstance(theta2, list):
                    a2, lp2 = self.act_aggregation(s2, theta2, meta_nash2)
                else:
                    a2, lp2 = self.act(s2, theta2)

                (s1, s2), (r1, r2),_,_ = self.imp.step((a1, a2))
    ...
    def br1(self, agent1, agent2_pop, metanash, train_iter, lr):
        for update in range(train_iter):
            ...
            dice_objective = dice_memory.dice_objective(self.gamma, self.use_baseline)/self.len_rollout
            grad = torch.autograd.grad(dice_objective, agent1.theta, create_graph= not self.test)[0]
            ...
            if update % 5==0:
                ...
                p2 = p1
                score = self.step(agent1.theta, agent2_pop, meta_nash2 = metanash)
        ...
        score = self.step(agent1.theta, agent2_pop, grad=True, meta_nash2 = metanash)
    ...
    def br2(self, agent2, agent1_pop, metanash, train_iter, lr):
        for update in range(train_iter):
            ...
            for t in range(self.len_rollout):
                ...
                a1, lp1 = self.act_aggregation(s1, agent1_pop, metanash)
                (s1, s2), (r1, r2),_,_ = self.imp.step((a1, a2))
                s2_one = np.eye(self.imp.NUM_STATES)[s2]
            ...
            dice_objective = dice_memory.dice_objective(self.gamma, self.use_baseline)/self.len_rollout
            grad = torch.autograd.grad(dice_objective, agent2.theta, create_graph= not self.test)[0]
            ...
            if update % 5==0:
                #continue
                p1 = [p.item() for p in torch.sigmoid(agent2.theta)]
                p2 = p1
                score = self.step(agent1_pop, agent2.theta, meta_nash1 = metanash)
                #print('update', update, 'score (%.3f,%.3f)' % (score[0], score[1]) , 'policy (agent1) = {%.3f, %.3f, %.3f, %.3f, %.3f}' % (p1[0], p1[1], p1[2], p1[3], p1[4]),' (agent2) = {%.3f, %.3f, %.3f, %.3f, %.3f}' % (p2[0], p2[1], p2[2], p2[3], p2[4]))
                
        score = self.step(agent1_pop, agent2.theta, grad=True, meta_nash1 = metanash)
            ...
            if not len(self.metagame) == self.pop_size:         
                if numpy:
                    ...
                    for i in range(len(self.pop2)):
                        agent1 = self.pop1[-1]
                        agent2 = self.pop2[i]
                        score = self.step(agent1.theta, agent2.theta)[0]
                    ...
                    for i in range(len(self.pop2)):
                        agent1 = self.pop1[-1]
                        agent2 = self.pop2[i]
                        score = self.step(agent1.theta, agent2.theta, grad=True)[0]
                        self.metagame[-1, i] = score
                    for j in range(len(self.pop2)-1):
                        agent1 = self.pop1[j]
                        agent2 = self.pop2[-1]

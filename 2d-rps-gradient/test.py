...
parser.add_argument('--psro_iters', type=int, default=15)
...
def run_experiment(mod_list, method_list, unique_params, num_unique, num_test_games, psro_iters):
    ...
    for i, mod in enumerate(mod_list):
        ...
        for j in tqdm(range(num_test_games)):
            game = game_list[i][j]
            exps = run_psro(game, args.environment, method=method_list[i], model=mod, iters=psro_iters, inner_lr=2, inner_iters=5, exp_inner_lr=0.5, exp_iters=75, implicit=False)
            model_exps[i].append(exps)
    
    d = model_exps
    tmp_label = 'test_result' + str(method_list[0])# + str(3)
    PATH_DATA = os.path.join(PATH_RESULTS, 'test_data')
    #PATH_DATA = os.path.join(PATH_DATA, 'step9')

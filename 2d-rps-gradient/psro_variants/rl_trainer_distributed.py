...
from environments.IMP import Agent, TorchPop_imp, sample_imp_games
...
def psro_subloop(proc, config, game_args_dict, model, game_sampler, meta_grad, stop_value, start_event, end_event, end_count, batch_losses, batch_norms):
    while stop_value == 0:
        ...
        for psro_iter in range(config['nb_psro_iters']):
            ...
            if (psro_iter + 1) % config['window_size'] == 0:
                ...
                final_exp = torch_pop.get_exploitability(meta_nash1, meta_nash2, config['nb_exp_iters'], config['inner_lr'])
                print('final exp got')

...
class TorchPop_rps:
    ...
    def psro_popn_update(self, meta_nash, train_iters, train_lr, lam, norm_break_val, implicit=False):
        if implicit == False:
            ...
            _, br_agent = best_response_trainer(agg_agent.x, agg_agent, lam, train_lr, train_iters, norm_break_val)
            self.add_agent()
    ...
    def get_exploitability(self, meta_nash, training_iters, norm_break_val, lr=1.0, test=False, implicit=False, lam=0.1):
        ...
        if implicit == False:
            ...
            exp, _ = exp_best_response_trainer(agg_agent.x, agg_agent, lam, lr, training_iters, norm_break_val)
        return 2 * exp
...
def get_best_response(GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val):
    torch.manual_seed(0)
    ...
    for _ in range(train_iters):
        ...
        inner_norms.append(torch.norm(br.x.grad).item())
        if len(inner_norms) > 10:
            if np.mean(inner_norms[-10:]) < norm_break_val:
                break
        opt.step()
    return br.x, np.mean(inner_norms[-10:])

class implicit_best_responder(nn.Module):
    def __init__(self):
        super(implicit_best_responder, self).__init__()
    def forward(self, GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val):
        br, norm = get_best_response(GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val)
...
class implicit_br_new(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val):
        torch.manual_seed(0)
        br = GMMAgent(GMM_agg_agent.mu, GMM_agg_agent.game, type_ag=GMM_agg_agent.type_ag, nn_input_size=GMM_agg_agent_logits.shape[0])
        br.x = (0.01*torch.randn(GMM_agg_agent_logits.shape[0], dtype=torch.float)).clone().detach().to(device_train)
        br.x.requires_grad = True
...
class implicit_best_responder_new(nn.Module):
    ...
    def forward(self, GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val):
        br = self.best(GMM_agg_agent_logits, GMM_agg_agent, lam, lr, train_iters, norm_break_val)
        r = get_payoff(br, GMM_agg_agent_logits, GMM_agg_agent.mu, GMM_agg_agent.game)
        return r, br      